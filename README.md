...
```bash
dk bash  # starts bash inside "vue-photo" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and vue-photo.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `vue-photo` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The vue-photo container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv vue-photo -p python3  # creates a python3 virtualenv
